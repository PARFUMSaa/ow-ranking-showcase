...
## 直近1か月のキャラ使用状況(更新差分から再構成)

- OverFast には「最近の試合」情報が無いため、`update.mjs` が**毎回の更新でヒーロー別の試合数の増加分**を記録し、
  直近 **30回分(≒1か月)** を `recent` としてプレイヤーデータに保持します
- サイト側は**直近1か月に含まれる更新分を合算**し、キャラ使用状況を**試合数の多い順に上位5件**表示します
  (プレイヤー詳細カードと、上部のプレイヤー要約の両方)
  - 表示はヒーローアイコン+試合数のチップ(背景の棒が1位を100%とした相対比。パーセントや注記は出しません)
  - 例: `📊 直近1か月のキャラ使用状況` / `Ana 16試合`
- 履歴は更新のたびに**前回の履歴を引き継いで加算**します(新規取得レコードへ履歴をコピー)
...
- 過去の git 履歴(自動更新コミット)からも差分を復元できるため、導入直後から数日分のデータがあります
  (旧仕様でヒーロー別プールが未生成だったスナップショットは自動で除外)

## ヒーロー順位の変動バッジ(▲ / ▼ / ★)

- `update.mjs` は更新時に、**生成前の `players-data.js`(前回のランキング)と比べて**
  プレイヤーごとのヒーロー順位が何位上がった/下がったかを計算し、各ヒーローへ書き込みます
  - `rankDelta` … **スコア順**(バーの長さ=素点×ランク係数)の順位変動(正=アップ)
  - `rankDeltaPower` … **Power Rating順**(finalScore)の順位変動(正=アップ)
  - `rankNew` … 前回のランキングに無かったヒーロー
- サイト側(Hero Breakdown)では、ヒーローアイコンの左に **▲1(アップ) / ▼2(ダウン) / ★(新登場)** の
  記号+数字だけのバッジとして表示します。**現在の並び順と同じ基準の値**を使い、ツールチップに
  「前回のランキング(スコア順/Power Rating順)より◯位上がりました」と表示します
- Hero Breakdown の並び順は **`Power Rating順`(既定) / `スコア順`** のボタンで切り替えできます。
  グラフも並び順に連動します
  - Power Rating順: バー=**最終Power Rating**(目盛り0〜160)・右端にPower値・行内に素点(ランク係数適用後)
  - スコア順: バー=**バトルスコア(素点)+ランク係数ぶん(「＞」が流れる)**・右端に「素点→ランク適用後」・行内にPower値
